///////////////////////////////////////////////////////////////////////////////
// File Name : 2021202058_preforked_server.c			    	     //
// Date :  2023/05/15			                                     //
...
// Description : Based on the previous task, the server is implemented in a  //
//	pre-forked manner. Create or maintain five child processors. The     //
//	parent process creates a child process through a fork function and   //
//	outputs information to the terminal. In the event of a SIGINT, all   //
//	processes must be completely terminated, and at this time, the       //
//	zombie process must not be created.	     			     //
///////////////////////////////////////////////////////////////////////////////
...
int nrequest = 0;               // 현재까지의 요청 수 저장
int trequest = 0;               // 전체 요청 수 저장
struct sockaddr_in history[10]; // 최근 10개의 연결 기록 저장
int PID[10];                    // 자식 프로세스의 PID 저장
time_t TIME[10];                // 시간 정보 저장
static int maxNchildren;        // 최대 자식 프로세스 수를 저장
...
//	connection history. The handle_signal function is a signal handler   //
//	that handles signals, SIGCHLD, SIGALRM, SIGINT, SIGUSR1, SIGTERM.    //
//	SIGCHLD is responsible for processing child process termination      //
//	events and SIGALRM is displaying connection history. SIGINT is the   //
//	signal generated when Ctrl+C is pressed in the terminal, and it      //
//	signals the end of the program. SIGUSR1 is one of the user-defined   //
//	signals, which is used to perform a specific action that the user    //
//	wants. SIGTERM refers to a signal requesting the process to terminate//
...
//	is being handled.						     //
///////////////////////////////////////////////////////////////////////////////
void handle_signal(int signal) // 시그널에 대한 핸들러
{
    int status;
    pid_t pid;
    ...
    if (signal == SIGALRM) // SIGALRM 시그널 핸들러
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            kill(pids[i], SIGUSR1); // 자식 프로세스에 SIGUSR1 시그널을 보내 연결 기록을 요청
        }
        alarm(10); // 10초마다 SIGALRM 시그널 발생
    }
    if (signal == SIGUSR1) // SIGUSR1 시그널 핸들러
    {
        int n = nrequest - 1; // 인덱스 n을 설정해 최근 연결 기록부터 출력
        int i = trequest;     // 인덱스 i를 설정해 trequest 부터 시작
        while (i > (trequest - nrequest))
        {
            printf("%d\t%s\t%d\t%d\t%s\t\n", i, inet_ntoa(history[n].sin_addr), getpid(), history[n].sin_port, ctime(&TIME[n])); // 연결 정보 출력
            i--;                                                                                                                 // 인덱스 감소
            n--;                                                                                                                 // 연결 기록의 이전 항목으로 이동
        }
    }
    ...
    {
        printf("\n");
        for (int i = 0; i < maxNchildren; i++) // 자식 프로세스 개수만큼
        {
            kill(pids[i], SIGTERM); // 자식 프로세스에 SIGTERM 시그널 보내 종료 요청
            int status;
            ...
            printf("[%s] %d process is terminated.\n", time_str, pids[i]); // 종료된 자식 프로세스 상태 출력
        }
        time_t current_time;                              // 현재 시간 저장 변수
        time(&current_time);                              // 현재 시간 저장
        char *time_str = ctime(&current_time);            // 변수의 값을 문자열로 변환하여 저장
...
{
    signal(SIGUSR1, handle_signal); // SIGUSR1 시그널에 대한 핸들러 설정
    ...
    while (1)
    {
        ...
        if (nrequest == 10)
        {
            ...
            {
                history[i - 1] = history[i]; // history 배열의 요소를 한 칸씩 앞으로 이동
                PID[i - 1] = PID[i];         // pid 배열의 요소를 한 칸씩 앞으로 이동
                TIME[i - 1] = TIME[i];       // time 배열의 요소를 한 칸씩 앞으로 이동
            }

            history[nrequest - 1].sin_addr = client_addr.sin_addr; // 가장 오래된 위치에 새로운 클라이언트의 IP 주소 저장
            history[nrequest - 1].sin_port = client_addr.sin_port; // 가장 오래된 위치에 새로운 클라이언트의 port 번호 저장
            PID[nrequest - 1] = pid;                               // 가장 오래된 위치에 현재 프로세스 ID 저장
            TIME[nrequest - 1] = time(NULL);                       // 가장 오래된 위치에 현재 시간을 저장
        }
        else
        {
            trequest++;                                        // 전체 요청 수 증가
            history[nrequest].sin_addr = client_addr.sin_addr; // 현재 요청의 클라이언트 IP 주소 저장
            history[nrequest].sin_port = client_addr.sin_port; // 현재 요청의 클라이언트 포트 번호 저장
            PID[nrequest] = pid;                               // 현재 프로세스의 ID를 저장
            TIME[nrequest] = time(NULL);                       // 현재 시간을 저장
            nrequest++;                                        // 현재 요청 수 증가
        }
        close(client_fd);
    }
...
int main(int argc, char **argv)
{
    ...
    maxNchildren = 5;                                     // 최대 자식 프로세스 수 5로 설정
    pids = (pid_t *)malloc(maxNchildren * sizeof(pid_t)); // 자식 프로세스 ID 저장할 배열 동적 할당
    ...
    for (int i = 0; i < maxNchildren; i++) // 최대 자식 프로세스 수만큼 반복
    {
