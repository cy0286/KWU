///////////////////////////////////////////////////////////////////////////////
// File Name : 2021202058_web_server.c				    	     //
// Date :  2023/05/03			                                     //
...
// Description : This program is a simple HTTP server written in C language. //
//	It uses socket programming to process client requests, receives      //
//	incoming connections, and returns the requested file to the client.  //
//	Parsing HTTP requests to determine the requested file, and if the    //
//	file exists, sends the file data to the client.   		     //
///////////////////////////////////////////////////////////////////////////////
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        inet_client_address.s_addr = client_addr.sin_addr.s_addr;                                         // 클라이언트 IP 주소 저장
        printf("[%s : %d] client was connected\n", inet_ntoa(inet_client_address), client_addr.sin_port); // 연결된 클라이언트의 IP와 포트 번호 출력
        read(client_fd, buf, BUFSIZE);                                                                    // 클라이언트로부터 데이터를 읽어옴
        strcpy(tmp, buf);                                                                                 // buf를 tmp에 복사
        puts("====================");
        printf("Request frmo [%s : %d]\n", inet_ntoa(inet_client_address), client_addr.sin_port); // 클라이언트 IP와 포트 번호 출력
        puts(buf);                                                                                // 클라이언트로부터 받은 데이터 출력
        puts("====================");
