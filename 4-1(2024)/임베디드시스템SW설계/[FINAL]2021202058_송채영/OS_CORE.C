...
// File name to store loan information
#define LOAN_RECORD_FILE "loan_records.txt"

// A function that records loan information to a file
void WriteLoanRecord(const char* bookTitle, const char* action)
{
    FILE* fp;
    fp = fopen(LOAN_RECORD_FILE, "a"); // Open files in extra mode
    ...
    if (fp != NULL)
    {
        ...
        // Write to a file
        fprintf(fp, "Timestamp: %s, Book Title: %s, Action: %s\n", time_str, bookTitle, action);
        fclose(fp);
    }
    else // Failed to open the file
    {
        printf("Error: Failed to open loan record file.\n");
    }
}
